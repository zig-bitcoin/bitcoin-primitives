...
//!
const Ripemd160 = @import("../../hashes/lib.zig").Ripemd160;
...
pub const ExtendedPrivKey = struct {
    ...
    pub fn fromStr(allocator: std.mem.Allocator, s: []const u8) !ExtendedPrivKey {
        const decoder = base58.Decoder{};
        const decoded = try decoder.decodeCheckAlloc(allocator, s);
    ...
    pub fn toStr(self: ExtendedPrivKey, allocator: std.mem.Allocator) ![]const u8 {
        ...
        const encoder = base58.Encoder{};

        return encoder.encodeCheckAlloc(allocator, &encoded);
    }
...
pub const ExtendedPubKey = struct {
    ...
    pub fn toStr(self: ExtendedPubKey, allocator: std.mem.Allocator) ![]const u8 {
        const encoder = base58.Encoder{};
        return try encoder.encodeCheckAlloc(allocator, &self.encode());
    }
...
test "base58_check_decode_encode" {
    const encoder = base58.Encoder{};
    const encoded = try encoder.encodeCheckAlloc(std.testing.allocator, "test");
    defer std.testing.allocator.free(encoded);

    const decoder = base58.Decoder{};
    const decoded = try decoder.decodeCheckAlloc(std.testing.allocator, encoded);
